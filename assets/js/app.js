(() => {
  ...
  const localeMessages = {
    ...
    mt:{invalid:'Jekk jogħġbok imla l-oqsma obbligatorji kollha u ċċekkja li l-passwords jaqblu.',signupOk:'Il-preview ġiet ivvalidata. Il-checkout tal-produzzjoni għadu mhux konness f’dan ir-repository.',demoOk:'It-talba ntbagħtet b’suċċess. Se nikkuntattjawk dalwaqt.',demoError:'It-talba ma setgħetx tintbagħat. Erġa’ pprova aktar tard.',demoSending:'Qed tintbagħat it-talba…',contact:'Ikkuntattjana',users:'Setup miftiehem',requestSetup:'Itlob setup',commercialOk:'Il-preview ġiet ivvalidata. Il-follow-up kummerċjali tal-produzzjoni għadu mhux konness f’dan ir-repository.',next2Title:'Follow-up kummerċjali',next2Text:'Nirrevedu miegħek il-pjan magħżul, id-daqs tat-tim u l-ħtiġijiet ta’ implementazzjoni.',next3Title:'Triq ta’ implementazzjoni',next3Text:'Naqblu fuq setup, appoġġ għall-onboarding u prijoritajiet ta’ integrazzjoni qabel l-attivazzjoni.'},
    de:{invalid:'Bitte füllen Sie alle Pflichtfelder aus und prüfen Sie, ob die Passwörter übereinstimmen.',signupOk:'Vorschau validiert. Der produktive Checkout ist in diesem Repository noch nicht angebunden.',demoOk:'Anfrage erfolgreich gesendet. Wir werden Sie in Kürze kontaktieren.',demoError:'Die Anfrage konnte nicht gesendet werden. Bitte versuchen Sie es später erneut.',demoSending:'Anfrage wird gesendet…',contact:'Kontakt aufnehmen',users:'Vereinbarte Konfiguration',requestSetup:'Einrichtung anfragen',commercialOk:'Vorschau validiert. Der produktive Endpunkt für die kommerzielle Abstimmung ist in diesem Repository noch nicht angebunden.',next2Title:'Kommerzielle Abstimmung',next2Text:'Wir prüfen mit Ihnen Tarif, Teamgröße und Implementierungsbedarf.',next3Title:'Implementierungsweg',next3Text:'Vereinbaren Sie Einrichtung, Onboarding-Support und Integrationsprioritäten vor der Aktivierung.'},
    es:{invalid:'Completa todos los campos obligatorios y comprueba que las contraseñas coincidan.',signupOk:'Vista previa validada. El checkout de producción todavía no está conectado en este repositorio.',demoOk:'Solicitud enviada correctamente. Nos pondremos en contacto contigo pronto.',demoError:'No se pudo enviar la solicitud. Inténtalo de nuevo más tarde.',demoSending:'Enviando solicitud…',contact:'Contactar',users:'Configuración acordada',requestSetup:'Solicitar configuración',commercialOk:'Vista previa validada. El endpoint de producción para seguimiento comercial todavía no está conectado en este repositorio.',next2Title:'Seguimiento comercial',next2Text:'Revisamos contigo el plan elegido, el tamaño del equipo y las necesidades de implantación.',next3Title:'Ruta de implantación',next3Text:'Acordamos configuración, soporte de onboarding y prioridades de integración antes de la activación.'},
    fr:{invalid:'Complétez tous les champs obligatoires et vérifiez que les mots de passe correspondent.',signupOk:'Aperçu validé. Le paiement de production n’est pas encore connecté dans ce dépôt.',demoOk:'Demande envoyée avec succès. Nous vous contacterons prochainement.',demoError:'La demande n’a pas pu être envoyée. Veuillez réessayer plus tard.',demoSending:'Envoi de la demande…',contact:'Nous contacter',users:'Configuration convenue',requestSetup:'Demander la configuration',commercialOk:'Aperçu validé. Le point de terminaison de production pour le suivi commercial n’est pas encore connecté dans ce dépôt.',next2Title:'Suivi commercial',next2Text:'Nous examinons avec vous l’offre choisie, la taille de l’équipe et les besoins de mise en œuvre.',next3Title:'Parcours de mise en œuvre',next3Text:'Convenez de la configuration, du support onboarding et des priorités d’intégration avant activation.'},
    ru:{invalid:'Заполните все обязательные поля и проверьте, что пароли совпадают.',signupOk:'Предварительная версия проверена. Рабочая платёжная интеграция в этом репозитории пока не подключена.',demoOk:'Запрос успешно отправлен. Мы свяжемся с вами в ближайшее время.',demoError:'Не удалось отправить запрос. Повторите попытку позже.',demoSending:'Отправка запроса…',contact:'Связаться с нами',users:'Согласованная конфигурация',requestSetup:'Запросить настройку',commercialOk:'Предварительная версия проверена. Рабочая интеграция для коммерческого сопровождения в этом репозитории пока не подключена.',next2Title:'Коммерческое сопровождение',next2Text:'Мы вместе проверим выбранный тариф, размер команды и потребности внедрения.',next3Title:'Путь внедрения',next3Text:'До активации согласуем настройку, поддержку онбординга и приоритеты интеграции.'}
  };

  const languageNames={en:'EN',it:'IT',mt:'MT',de:'DE',es:'ES',fr:'FR',ru:'RU'};
  ...
  function showPage(route){
    ...
    const mobileToggle=document.querySelector('.mobile-toggle,.mobile-menu-button');
  ...
  function prepareLanguageMenu(){
    ...
    if(label) label.textContent=languageNames[lang]||'EN';
  ...
  function prepareMobileNav(){
    const toggle=document.querySelector('.mobile-toggle,.mobile-menu-button');
  ...
  function validateForm(form){
    ...
    if(password&&confirm&&password.value!==confirm.value){valid=false;confirm.setCustomValidity('Passwords do not match');}
  ...
  function prepareForms(){
    ...
    signup?.addEventListener('submit',event=>{
      ...
      const copy=localeMessages[lang]||localeMessages.en;
      if(!validateForm(signup)){if(status) status.textContent=copy.invalid;return;}
      const selected=document.querySelector('input[name="selectedPlan"]:checked')?.value||'Essential';
  ...
  function readPreferences(){
    try{const raw=localStorage.getItem(preferenceKey);return raw?JSON.parse(raw):null;}catch{return null;}
  }
  function storePreferences(analytics,marketing){
    const value={essential:true,analytics:Boolean(analytics),marketing:Boolean(marketing),updatedAt:new Date().toISOString()};
    try{localStorage.setItem(preferenceKey,JSON.stringify(value));}catch{}
    html.dataset.analyticsConsent=String(value.analytics);
    html.dataset.marketingConsent=String(value.marketing);
    return value;
  }
  ...
  function preparePrivacyControls(){
    ...
    const apply=prefs=>{
      if(!prefs) return;
      html.dataset.analyticsConsent=String(Boolean(prefs.analytics));
      html.dataset.marketingConsent=String(Boolean(prefs.marketing));
      if(analytics) analytics.checked=Boolean(prefs.analytics);
      if(marketing) marketing.checked=Boolean(prefs.marketing);
    };
    const openModal=()=>{apply(readPreferences()||{analytics:false,marketing:false});if(modal) modal.hidden=false;document.body.classList.add('modal-open');close?.focus();};
    const closeModal=()=>{if(modal) modal.hidden=true;document.body.classList.remove('modal-open');};
    const existing=readPreferences();
    if(existing) apply(existing); else if(banner) banner.hidden=false;
    ...
    document.getElementById('saveCookieSettings')?.addEventListener('click',()=>{storePreferences(analytics?.checked,marketing?.checked);if(banner) banner.hidden=true;closeModal();});
